...
valid'' (Rel s1 ps1) (Rel s2 ps2) = s1 /= s2 || length ps1 /= length ps2 || any (uncurry_valid'') (zip ps1 ps2)
valid'' _ _ = True

valid' :: Diseq -> Bool
valid' = all (uncurry_valid'')

valid :: Interpretation -> Bool
valid (Interpretation _ _ diseq) = valid' diseq

uncurry_valid'' = uncurry valid''
...
logSymInitChar :: ReadP Char
logSymInitChar = choice (char `map` (['a' .. 'z'] ++ ['A' .. 'Z']))

logSymChar :: ReadP Char
logSymChar = choice (logSymInitChar : (char `map` (['0' .. '9'] ++ ['-'])))

logSymbol :: ReadP [Char]
logSymbol = do
    c <- logSymInitChar
    cs <- many logSymChar
    return (c : cs)

logName :: ReadP SimpleProposition
logName = do
    n <- logSymbol
    return (Name n)

logVar :: ReadP SimpleProposition
logVar = do
    char '#'
    n <- logSymbol
    return (Var n)

logRel :: ReadP SimpleProposition
logRel = do
    char '['
    rn <- logSymbol
    string ": "
...
logSExp :: ReadP SimpleProposition
logSExp = choice [logName, logVar, logRel]

logCSExpList0 :: ReadP [SimpleProposition]
logCSExpList0 = do
    string ", "
    x <- logSExp
    xs <- logSExpList0
...
logSExpList0 :: ReadP [SimpleProposition]
logSExpList0 = choice [logCSExpList0, return []]

logSExpList :: ReadP [SimpleProposition]
logSExpList = do
    x <- logSExp
    xs <- logSExpList0
    return (x : xs)
...
logExp :: ReadP Proposition
logExp = choice [logSExp >>= (return . Simple), logEq, logNeq]

logCExpList0 :: ReadP [Proposition]
logCExpList0 = do
    string ", "
    x <- logExp
    xs <- logExpList0
...
logExpList0 :: ReadP [Proposition]
logExpList0 = choice [logCExpList0, return []]

logExpList1 :: ReadP [Proposition]
logExpList1 = do
    x <- logExp
    xs <- logExpList0
    return (x : xs)

logExpList :: ReadP [Proposition]
