...
import Control.Monad (liftM, forM, forM_, replicateM)
...
--        print (p1, p2)
        ...
        p1G <- MV.read sMV p1
        p2G <- MV.read sMV p2
        -- instead of holding lists, just hold length
        p1GL <- MV.read grpMV p1G
        p2GL <- MV.read grpMV p2G
        -- create list of prisoners pointing to group p2G
        
        -- this solution was good excercise for Monads, but is much slower
        -- than mutable version holding and rearranging lists, because it 
        -- needs to go through whole list of prisoners to get a list
        -- for those to redirect (merge)
        
        toRedirect <- fmap concat $ forM [1 .. m] $ (\p -> do
            p2G_ <- MV.read sMV p
            return $ if p2G == p2G_ then [p] else []
            ) -- :: IO [Int]
        let t = toRedirect :: [Int]
        ...
        case () of
            _
                ...
                | 1 == p1GL && 1 == p2GL -> do
                    MV.write sMV p2 p1G
                    MV.write grpMV p1G 2
                    MV.write grpMV p2G 0
                -- both linked and in same group -> nothing to do
                | 1 /= p1GL && 1 /= p2GL && p1G == p2G -> return ()
                -- p1 linked and p2 linked, different groups -> merge groups to p1G
                | 1 /= p1GL && 1 /= p2GL -> do
                    -- all pointing to p2G -> redirect to p1G
                    forM_ toRedirect $ (\x -> MV.write sMV x p1G)
...
--                        ++ " p1GL " ++ show p1GL ++ " p2GL " ++ show p2GL
                ...
                | 1 /= p1GL -> do
                    MV.write sMV p2 p1G
                    MV.write grpMV p1G (p1GL + 1)
                    MV.write grpMV p2G 0
                -- just p2 is linked -> link p1 to p2G
                | 1 /= p2GL -> do
                    MV.write sMV p1 p2G
                    MV.write grpMV p2G (p2GL + 1)
                    MV.write grpMV p1G 0
        )
    grpVRes <- V.freeze grpMV
    let costs' = V.sum $ V.map (gCost') grpVRes
        gCost' :: Int -> Int
        gCost' x = (fromEnum sqrtX) + roundUp
            where
            sqrtX = sqrt (toEnum x)
