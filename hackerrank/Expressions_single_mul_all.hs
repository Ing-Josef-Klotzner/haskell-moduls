...
makeExpressions2 t intL = (go bnrx lMap lMap bcnr bmuIL bmnr_keys bmmuIL hbnk)
    where
    bnrx = 50 `quot` bnr + 1
    bnr = head bmuIL
    bnnr = head $ tail bmuIL
    bmuIL = M.keys lMap
    bmmuIL
        | ibnr_keys = bmuIL
        | otherwise = tail bmuIL
    hbnk
        | ibnr_keys = [head bmnr_keys] ++ bmnr_keys
        | otherwise = [head bmnnr_keys] ++ bmnnr_keys
    bmnr_keys = filter (valid bnrMap) $ M.keys bnrMap
    bmnnr_keys = filter (valid bnnrMap) $ M.keys bnnrMap
    ibnr_keys = bmnr_keys /= []
    bnrMap = lMap M.! bnr
    bnnrMap = lMap M.! bnnr
    ...
    go 0 sMap mMap mcnr muIL mnr_keys mmuIL mmnr_keys = go nnrx sMap mMap mcnr upmuIL nmnr_keys mmuIL mmnr_keys
        where
        upmuIL = tail muIL
        nnr
    ...
    go n sMap mMap mcnr muIL [] mmuIL mmnr_keys = go nnrx sMap mMap mcnr upmuIL nmnr_keys mmuIL mmnr_keys
        where
        upmuIL = tail muIL
        nnr
        ...
        nnrMap
            ...
            | otherwise = M.empty
        nnrx = 50 `quot` nnr + 1
    go n sMap mMap mcnr muIL mnr_keys mmuIL mmnr_keys
        | dcalcChkL = if dcalcChkL == calcChkL 
                    then (zipC_ $ mapToL mMap) else "wrong result " ++ (zipC_ $ mapToL mMap)
        | mcnr < (-101) = gom sMap mmuIL mmnr_keys
--        | otherwise = show x ++ " " ++ show xp ++ " " ++ show mcnr ++ " c" ++ show ccnr ++ " " ++ (go (n - 1) sMap modi ccnr muIL upmnr_keys mmuIL)
        | otherwise = go (n - 1) sMap modi ccnr muIL upmnr_keys mmuIL mmnr_keys
        where
        -- finding difference of result to 0 and find out which number to subtract to get result
        -- there must not be result, which is in list !!
        mnr_key 
        ...
        xp = 101 - x
--            | even (101 - dc1) = (101 - dc1) `quot` 2
--            | otherwise = (202 - dc1) `quot` 2
        dcalcChkL = dc1 == 0
        dc1 = mcnr `rem` 101
        dcnr = mcnr - 2 * nr
        calcChkL = c1 == 0
    ...
    gom pmMap [] mmnr_keys = "to be written multiple '*'"
    gom pmMap mmuIL [] = gom pmMap mmmuIL mmmnr_keys
        where
        mmmuIL = tail mmuIL
        mmmnr_keys = filter vald $ M.keys nnrMap
        vald k = k /= 0
        nnr
            | mmmuIL /= [] = head mmmuIL
            | otherwise = (-1)
        nnrMap
            | nnr /= (-1) = pmMap M.! nnr
            | otherwise = M.empty
    gom pmMap mmuIL mmnr_keys = (go bmnrx mmMap mmMap bmcnr bmmuIL bmmnr_keys mmmuIL mmmnr_keys)
        where
        ...
        mmmuIL
            | ipnr_keys = mmuIL
            | otherwise = tail mmuIL
        mmmnr_keys = tail mmnr_keys
        nr = head mmuIL
        rmMap = M.insert nr updt_nrMap pmMap
        updt_nrMap = M.insert nr_key '+' nrMap
        nr_key = head mmnr_keys
        nrMap = pmMap M.! nr
        mmMap
            | ipnr_keys = M.insert nr updt_pnrMap rmMap
            | innr_key = M.insert nnr updt_nnrMap rmMap
            | otherwise = rmMap
        updt_pnrMap = M.insert pnr_key '*' nrMap
        pnr_key = head mmmnr_keys
        updt_nnrMap = M.insert nnr_key '*' nnrMap
        nnr_key = head nnr_keys
        ipnr_keys = mmmnr_keys /= []
        innr_key = nnr_keys /= []
        nnr
...
-- "5-6+7*13+17+23+33-41+43+49"
-- *Main> makeExpressions2 18 [1,1,1,1,1,1,19,19,19,19,19,1,1,2,2,3,3,14]
--"1-1-1-1-1-1+19+19+19+19+19+1+1*2+2-3+3+14"
--7
--33 55 45 67 74 54 48
--33+55+45+67*74-54*48
--makeExpressions2 10 [46,47,48,49,50,51,51,52,53,54]
--"to be written multiple '*'"
-- could be "46+47+48+49+50*51-51*52-53*54"
-- *Main> makeExpressions2 7 [33,55,45,67,74,54,48]
--"to be written multiple '*'"
--Main> makeExpressions2 7 [51,51,45,54,74,54,48]
--"51*51+45+54+74+54*48"  -- wrongly 2 * but correct result
