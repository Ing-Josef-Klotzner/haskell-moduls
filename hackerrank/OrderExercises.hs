...
kadaneL inputV k = go sm1 sm1 1 V.empty where
    ...
    go sm meh si kadL
        ...
        maxNnewStart = gog 0 meh si
            where
            gog smg mehg sig
                | sig == len || newStartg = smg
...
{-def max_subarray(A):
    ...
    return max_so_far -}
shorten :: V.Vector Int -> Int -> Int -> Int -> V.Vector Int
shorten inputV si lg sm = lftPrt V.++ rgtPrt
    where
    lftPrt
        | isInit = V.empty
        | True = lftSlc
    rgtPrt
        | isLast = V.empty
        | True = rgtSlc
    lftSlc = V.slice 0 si inputV
    rgtSlc = V.slice (si + lg) (V.length inputV - si - lg) inputV
    isInit = V.sum lftSlc == sm
    isLast = V.sum rgtSlc == sm  
...
bgstL inputV k = go [] inputV k where
    go bLst inputV_ k_
        ...
        sInputV = shorten inputV_ si lg sm

-- O (n * k)
bgstLKadane :: V.Vector Int -> Int -> [Int]
bgstLKadane inputV k = go V.empty inputV k where
    go bLst inputV_ k_
        | k_ /= 0 && bLstN /= bLst && sm > 0 = go bLstN sInputV (k_ - 1)
        | True = V.toList bLst
        where
        bLstN = V.snoc bLst sm
...
--bgstLKadane inputV k = go V.empty inputV k where
--    go bLst inputV_ k_
--        | k_ /= 0 && bLstN /= bLst && sm > 0 = go bLstN sInputV (k_ - 1)  -- > 0
--        | True = V.toList inputV_
--        where
--        bLstN = V.snoc bLst sm
--        (sm, si, lg) = kadane inputV_
--        sInputV = shorten inputV_ si lg sm
