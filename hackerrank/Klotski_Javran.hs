...
search puz todoList visited = case Seq.viewl todoList of
    ...
    (b,mvs) Seq.:< bs ->
        ...
        _ | digestBox b `S.member` visited -> search puz bs visited
            -- a solution is found
        _ | fst targetBlock == targetCoord puz -> (b,mvs)
            -- expand the current queue, and search next one in the queue
        _ -> search puz (bs Seq.>< nextMoves (pzM puz) (pzN puz) (b,mvs)) newVisited
...
main = do
    ...
    let moves = reverse
              . snd
              $ search pz (return (pzBox pz,[])) S.empty
    print (length moves)
